...
from lib.data import get_coco_api_from_dataset
from lib.utils.misc import dist
from .dinod_engine import  train_one_epoch, evaluate
...
class DINODSolver(BaseSolver):
    def fit(self, ):
        ...
        n_t_parameters = sum(p.numel() for p in self.model.parameters())
        print(f'Number of Training Params: {n_parameters}')
        print(f'Number of Total Params : {n_t_parameters}')
        ...
        for epoch in range(self.last_epoch+1, args.TRAIN.epochs):
            ...
            if self.output_dir:
                ...
                if (epoch + 1) % args.TRAIN.checkpoint_step == 0:
